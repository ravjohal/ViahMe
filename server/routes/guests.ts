...
import { insertHouseholdSchema, insertGuestSchema, insertInvitationSchema, insertGuestSuggestionSchema } from "@shared/schema";
...
export async function registerGuestRoutes(router: Router, storage: IStorage) {
  ...
  router.get("/suggestions/:weddingId", await requireAuth(storage, false), ensureCoupleAccess(storage, (req) => req.params.weddingId), async (req, res) => {
    try {
      const { status } = req.query;
      let suggestions = await storage.getGuestSuggestionsByWedding(req.params.weddingId);
      ...
      res.status(500).json({ error: "Failed to fetch guest suggestions" });
    }
  });

  router.get("/suggestions/:weddingId/count", await requireAuth(storage, false), ensureCoupleAccess(storage, (req) => req.params.weddingId), async (req, res) => {
    try {
      const suggestions = await storage.getGuestSuggestionsByWedding(req.params.weddingId);
      const pendingCount = suggestions.filter(s => s.status === 'pending').length;
      res.json({ count: pendingCount, total: suggestions.length });
    } catch (error) {
      res.status(500).json({ error: "Failed to count suggestions" });
    }
  });

  router.post("/suggestions/:weddingId", await requireAuth(storage, false), async (req, res) => {
    try {
      const authReq = req as AuthRequest;
      if (!authReq.session.userId) {
        return res.status(401).json({ error: "Authentication required" });
      ...
      const validatedData = insertGuestSuggestionSchema.parse({
        ...req.body,
        weddingId,
        suggestedById: authReq.session.userId,
      });
      ...
      res.status(500).json({ error: "Failed to create guest suggestion" });
    }
  });

  router.post("/suggestions/:id/approve", await requireAuth(storage, false), async (req, res) => {
    try {
      const authReq = req as AuthRequest;
      if (!authReq.session.userId) {
        return res.status(401).json({ error: "Authentication required" });
      ...
      const wedding = await storage.getWedding(suggestion.weddingId);
      if (!wedding) {
        return res.status(404).json({ error: "Wedding not found" });
      }
      if (wedding.coupleId !== authReq.session.userId) {
        const roles = await storage.getWeddingRoles(suggestion.weddingId);
        const hasAccess = roles.some(role => role.userId === authReq.session.userId);
        if (!hasAccess) {
          return res.status(403).json({ error: "Access denied" });
        }
      ...
      const updatedSuggestion = await storage.updateGuestSuggestion(req.params.id, { status: 'approved' });
  ...
  router.post("/suggestions/:id/reject", await requireAuth(storage, false), async (req, res) => {
    try {
      const authReq = req as AuthRequest;
      if (!authReq.session.userId) {
        return res.status(401).json({ error: "Authentication required" });
      ...
      const updatedSuggestion = await storage.updateGuestSuggestion(req.params.id, { status: 'declined' });
      res.json(updatedSuggestion);
    } catch (error) {
  ...
  router.patch("/suggestions/:id/status", await requireAuth(storage, false), async (req, res) => {
    try {
      const authReq = req as AuthRequest;
      if (!authReq.session.userId) {
        return res.status(401).json({ error: "Authentication required" });
      ...
      const updatedSuggestion = await storage.updateGuestSuggestion(req.params.id, { status });
      res.json(updatedSuggestion);
    } catch (error) {
