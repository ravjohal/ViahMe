...
import { insertGuestListScenarioSchema, insertGuestBudgetSettingsSchema, insertCutListItemSchema } from "@shared/schema";
...
export function createCutListRouter(storage: IStorage): Router {
  const router = Router();
  ...
  router.post("/weddings/:weddingId/bulk-by-priority", async (req, res) => {
    const userId = req.session?.userId;
    if (!userId) {
      return res.status(401).json({ error: "Authentication required" });
    }
