...
export async function registerGuestGameRoutes(router: Router, storage: IStorage) {
  // Middleware to validate household magic token
  async function validateHouseholdToken(req: Request, res: Response, next: NextFunction) {
    ...
    const household = await storage.getHouseholdByToken(token as string);
    if (!household) {
