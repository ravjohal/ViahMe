...
function parseHtmlListings(html: string): VendorData[] {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    const location = locationMatch ? locationMatch[1].replace(/\s+/g, ' ').replace(/Serving customers in\s*/i, '').trim() : 'San Francisco Bay Area';
...
async function main() {
  console.log("Starting Entertainment vendor import...");
  ...
  console.log(`\nSuccessfully imported ${imported} new Entertainment vendors!`);
  process.exit(0);
}
