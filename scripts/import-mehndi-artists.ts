...
function parseHtmlListings(html: string): VendorData[] {
  ...
  // Use split approach and get content between listings
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    const locationMatch = listing.match(/location_on<\/span><b>\s*([\s\S]*?)<\/b>/i) ||
                          listing.match(/<div class="subtitle">[\s\S]*?<b>([^<]+)/i);
    const location = locationMatch ? locationMatch[1].replace(/\s+/g, ' ').replace(/Serving customers in\s*/i, '').trim() : 'Bay Area';
    ...
    const expMatch = listing.match(/work_history<\/span>([^<]+)/i) ||
                     listing.match(/<div class="experience">[\s\S]*?work_history[\s\S]*?<\/span>([^<]+)/i);
    const experience = expMatch ? expMatch[1].trim() : null;
    
    let description = '';
    const descMatch = listing.match(/<b>From the Business:<\/b>([^<]+)/i) ||
                      listing.match(/<p><b>From the Business:<\/b>([\s\S]*?)<\/p>/i);
    if (descMatch) {
...
function getCityFromLocation(location: string, name: string): string {
  ...
  if (loc.includes('bay area') || loc.includes('fremont') || loc.includes('san francisco') || 
      ...
      loc.includes('san mateo') || loc.includes('dublin') || loc.includes('berkeley') ||
      loc.includes('emeryville') || loc.includes('petaluma') || loc.includes('livermore') ||
...
async function main() {
  console.log("Starting Mehndi Artist vendor import...");
  
  const sql = neon(process.env.DATABASE_URL!);
  
  const existingVendors = await sql`SELECT name FROM vendors WHERE category = 'mehndi_artist'`;
  const existingNames = new Set(existingVendors.map(v => v.name.toLowerCase().trim()));
  console.log(`Found ${existingNames.size} existing mehndi artists in database`);
  
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766034286012.txt');
    ...
    try {
      ...
      await sql`
        INSERT INTO vendors (
          ...
          ${vendor.name},
          'mehndi_artist',
          ARRAY['mehndi_artist', 'henna_artist'],
          ${vendor.location},
          ${city},
          '$$',
          ARRAY['punjabi', 'hindi', 'gujarati', 'arabic'],
          ${vendor.description || 'Professional mehndi and henna services for South Asian weddings and events.'},
  ...
  console.log(`\nSuccessfully imported ${imported} new Mehndi Artist vendors!`);
  process.exit(0);
}
