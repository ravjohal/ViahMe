...
function parseHtmlListings(html: string): VendorData[] {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (listing.match(/Sangeet/i)) services.push('sangeet');
    if (listing.match(/Wedding\s*Receptions?/i)) services.push('reception');
    if (listing.match(/Engagement/i)) services.push('engagement');
...
async function main() {
  console.log("Starting Sacramento Wedding Coordinators import...");
  ...
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766081719635.txt');
  ...
  console.log(`\nSuccessfully imported ${imported} new Sacramento Wedding Coordinator vendors!`);
