...
async function main() {
  console.log('Starting invitation vendors import...');
  ...
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766133496890.txt');
  const html = fs.readFileSync(htmlPath, 'utf-8');

  // Extract invitation vendor names from title attributes
  const nameRegex = /title="Indian Invitation Printing Services - ([^"]+)"/gi;
  ...
  console.log(`Found ${uniqueNames.size} unique invitation vendor names to import`);

  // Extract city info for each vendor
  const cityMap = new Map<string, string>();
  const hrefRegex = /href="\/([a-z-]+)-ca\/invitation-printing-service\/([^"]+)"/gi;
    ...
    try {
      await sql`
        INSERT INTO vendors (
          ...
          ${name},
          'invitations',
          ARRAY['invitations', 'stationery'],
          ${city + ', CA'},
          'San Francisco Bay Area',
          '$$',
          ARRAY['indian', 'south_asian'],
          'Professional wedding invitation design and printing services specializing in South Asian wedding cards, scroll invitations, and custom stationery.',
          null,
          0,
          true,
          false,
  ...
  console.log(`\nSuccessfully imported ${imported} invitation vendors (${skipped} duplicates skipped)!`);
