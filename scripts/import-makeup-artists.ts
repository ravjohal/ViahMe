...
function getCityFromLocation(location: string, name: string): string {
  ...
  if (loc.includes('bay area') || loc.includes('fremont') || loc.includes('san francisco') || 
      ...
      loc.includes('mountain view') || loc.includes('stockton') || loc.includes('tracy')) {
...
async function main() {
  console.log("Starting Makeup Artist vendor import...");
  
  const sql = neon(process.env.DATABASE_URL!);
  
  const existingVendors = await sql`SELECT name FROM vendors WHERE category = 'makeup_artist'`;
  const existingNames = new Set(existingVendors.map(v => v.name.toLowerCase().trim()));
  console.log(`Found ${existingNames.size} existing makeup artists in database`);
  
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766034495995.txt');
    ...
    try {
      ...
      await sql`
        INSERT INTO vendors (
          ...
          ${vendor.name},
          'makeup_artist',
          ARRAY['makeup_artist', 'beautician', 'bridal_makeup'],
          ${vendor.location},
          ${city},
          '$$',
          ARRAY['punjabi', 'hindi', 'gujarati'],
          ${vendor.description || 'Professional makeup and beauty services for South Asian weddings and events.'},
  ...
  console.log(`\nSuccessfully imported ${imported} new Makeup Artist vendors!`);
  process.exit(0);
}
