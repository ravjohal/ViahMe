...
function mapCategory(category: string): string {
  ...
  if (cat.includes('rental') || cat.includes('infrastructure')) return 'rentals';
...
function mapCity(location: string): string {
  ...
  if (loc.includes('modesto') || loc.includes('turlock') || loc.includes('stockton')) return 'Fresno';
  if (loc.includes('sacramento')) return 'Sacramento';
  return 'Fresno';
}

async function main() {
  console.log('Starting Yuba City/Modesto vendors import...');
  ...
  const jsonPath = path.join(process.cwd(), 'attached_assets/Pasted--yuba-city-marysville-vendors-notes-The-premier-luxury-_1766137853129.txt');
  let content = fs.readFileSync(jsonPath, 'utf-8');

  // Extract vendor objects using regex since JSON is malformed
  const vendorRegex = /\{\s*"business_name":\s*"([^"]+)"[^}]*"category":\s*"([^"]+)"[^}]*"location":\s*"([^"]+)"[^}]*(?:"website":\s*"([^"]*)")?[^}]*(?:"notes":\s*"([^"]*)")?[^}]*\}/gi;
