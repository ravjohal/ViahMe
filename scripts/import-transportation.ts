...
async function main() {
  console.log('Starting transportation import...');
  ...
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766132364277.txt');
  const html = fs.readFileSync(htmlPath, 'utf-8');

  const listingRegex = /<div class="single-listing\s*[^"]*"\s*>/gi;
  const listings = html.split(listingRegex).slice(1);
  
  console.log(`Found ${listings.length} transportation listings to parse`);
  ...
  for (const listing of listings) {
    ...
    name = name.replace(/^Indian Wedding Transportation Services\s*-\s*/i, '').trim();
    ...
    let description = descMatch ? descMatch[1].trim().substring(0, 500) : 'Professional wedding transportation including horse carriages and baraat services for South Asian weddings.';
  ...
  console.log(`\nSuccessfully imported ${imported} transportation vendors (${skipped} duplicates skipped)!`);
