...
function parseHtmlListings(html: string): VendorData[] {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    let location = locationMatch ? locationMatch[1].replace(/\s+/g, ' ').trim() : 'Bay Area';
    
    let city = 'SF Bay Area';
    const locLower = location.toLowerCase();
    if (locLower.includes('fremont') || locLower.includes('san jose') || locLower.includes('sunnyvale') || 
        locLower.includes('daly city') || locLower.includes('stockton') || locLower.includes('bay')) {
      city = 'SF Bay Area';
    } else if (locLower.includes('new york') || locLower.includes('nyc') || locLower.includes('jersey')) {
      city = 'New York City';
    } else if (locLower.includes('los angeles') || locLower.includes('la ') || locLower.includes('hollywood')) {
      city = 'Los Angeles';
    } else if (locLower.includes('chicago')) {
      city = 'Chicago';
    } else if (locLower.includes('seattle')) {
      city = 'Seattle';
    } else if (locLower.includes('fresno')) {
      city = 'Fresno';
    }
...
async function main() {
  console.log("Starting Hair Stylist vendor import...");
  
  const sql = neon(process.env.DATABASE_URL!);
  
  const existingVendors = await sql`SELECT name FROM vendors WHERE category = 'hair_stylist'`;
  const existingNames = new Set(existingVendors.map(v => v.name.toLowerCase().trim()));
  console.log(`Found ${existingNames.size} existing hair stylists in database`);
  
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766035805590.txt');
    ...
    try {
      await sql`
        INSERT INTO vendors (
          ...
          ${vendor.name},
          'hair_stylist',
          ARRAY['hair_stylist', 'beautician', 'bridal_services'],
          ${vendor.location},
          ${vendor.city},
          '$$',
          ARRAY['punjabi', 'hindi', 'gujarati', 'tamil', 'telugu'],
          ${vendor.description || 'Professional hair styling and beauty services for South Asian weddings and events.'},
      ...
      imported++;
      if (imported % 50 === 0) {
        console.log(`Imported ${imported} vendors so far...`);
      }
    } catch (error) {
  ...
  console.log(`\nSuccessfully imported ${imported} new Hair Stylist vendors!`);
  process.exit(0);
}
