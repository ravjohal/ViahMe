...
async function main() {
  console.log("Starting Fresno DJ vendor import...");
  
  const sql = neon(process.env.DATABASE_URL!);
  
  const existingVendors = await sql`SELECT name FROM vendors WHERE category = 'dj'`;
  const existingNames = new Set(existingVendors.map(v => v.name.toLowerCase().trim()));
  console.log(`Found ${existingNames.size} existing DJs in database`);
  
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766034721772.txt');
    ...
    try {
      await sql`
        INSERT INTO vendors (
          ...
          ${vendor.name},
          'dj',
          ARRAY['dj', 'entertainment', 'music'],
          ${vendor.location},
          'Fresno',
          '$$',
          ARRAY['punjabi', 'hindi', 'gujarati', 'tamil', 'telugu'],
          ${vendor.description || 'Professional DJ and entertainment services for South Asian weddings and events.'},
  ...
  console.log(`\nSuccessfully imported ${imported} new Fresno DJ vendors!`);
  process.exit(0);
}
