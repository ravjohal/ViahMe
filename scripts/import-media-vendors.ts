...
function mapCategory(category: string): string {
  ...
  if (cat.includes('photo') && cat.includes('video')) return 'photography';
  ...
  if (cat.includes('multimedia') || cat.includes('agency')) return 'photography';
  return 'decor';
}

function mapCity(location: string): string {
  const loc = location.toLowerCase();
  if (loc.includes('yuba') || loc.includes('marysville')) return 'Yuba City';
  if (loc.includes('fremont') || loc.includes('bay area')) return 'Bay Area';
  if (loc.includes('sacramento') || loc.includes('roseville') || loc.includes('folsom') || loc.includes('elk grove') || loc.includes('citrus heights') || loc.includes('rancho cordova')) return 'Sacramento';
  if (loc.includes('fresno') || loc.includes('clovis') || loc.includes('modesto') || loc.includes('turlock') || loc.includes('central valley')) return 'Fresno';
  if (loc.includes('redding')) return 'Sacramento';
  if (loc.includes('sonoma')) return 'Bay Area';
  return 'Sacramento';
}

async function main() {
  console.log('Starting media vendors import...');
  ...
  const jsonPath = path.join(process.cwd(), 'attached_assets/Pasted--wedding-decorators-and-rentals-notes-Husband-and-wife-_1766138773297.txt');
