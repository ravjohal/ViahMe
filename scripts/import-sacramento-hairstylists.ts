...
function parseHtmlListings(html: string): VendorData[] {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    const location = locationMatch ? locationMatch[1].replace(/\s+/g, ' ').trim() : 'Sacramento, CA';
...
async function main() {
  console.log("Starting Sacramento Hair Stylist vendor import...");
  ...
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766035899535.txt');
  ...
  console.log(`\nSuccessfully imported ${imported} new Sacramento Hair Stylist vendors!`);
  process.exit(0);
}
