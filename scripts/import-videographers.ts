...
function parseHtmlListings(html: string): VendorData[] {
  ...
  while ((match = listingRegex.exec(html)) !== null) {
    ...
    const locationMatch = listing.match(/location_on<\/span>([^<]+)/i) ||
                          listing.match(/<div class="experience">[\s\S]*?location_on[\s\S]*?<\/span>([^<]+)/i);
    const location = locationMatch ? locationMatch[1].replace(/\s+/g, ' ').trim() : 'Bay Area';
    ...
    const ratingMatch = listing.match(/<div class="rating"><a[^>]*>(\d+)/i) ||
                        listing.match(/<div class="mob-rating"><a[^>]*>(\d+)/i);
...
async function main() {
  console.log("Starting Videographer vendor import...");
  ...
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766034043677.txt');
    ...
    try {
      ...
      await sql`
        INSERT INTO vendors (
          ...
          ${vendor.name},
          'videographer',
          ARRAY['videographer', 'cinematographer'],
          ${vendor.location},
          ${city},
          '$$',
  ...
  console.log(`\nSuccessfully imported ${imported} new Videographer vendors!`);
  process.exit(0);
}
