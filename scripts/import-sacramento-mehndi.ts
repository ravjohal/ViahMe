...
async function main() {
  console.log("Starting Sacramento Mehndi Artist vendor import...");
  ...
  const htmlPath = path.join(process.cwd(), 'attached_assets/Pasted--div-id-listing-div-xmlns-xlink-http-www-w3-org-1999-xl_1766036152034.txt');
    ...
    try {
      await sql`
        INSERT INTO vendors (
          ...
          ${vendor.name},
          'mehndi_artist',
          ARRAY['mehndi_artist', 'henna', 'bridal_mehndi'],
          ${vendor.location},
          'Sacramento',
          '$$',
          ARRAY['punjabi', 'hindi', 'gujarati', 'rajasthani', 'arabic'],
  ...
  console.log(`\nSuccessfully imported ${imported} new Sacramento Mehndi Artist vendors!`);
  process.exit(0);
}
