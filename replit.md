...
- **Ceremony Types System**: Database-driven ceremony definitions (table: `ceremony_templates`) with cost breakdowns via `ceremony_template_items` table. TypeScript exports use `ceremonyTypes`, `CeremonyType`, `ceremonyTypeItems`, `CeremonyTypeItem` naming while maintaining backward compatibility aliases. Dual API endpoints (`/api/ceremony-types` and `/api/ceremony-templates`) support both new and legacy clients.
