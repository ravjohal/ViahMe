...
- **Database-Driven Metro-City Mapping**: The `metro_cities` table maps 222 cities to 18 metro areas, loaded at startup into `server/utils/metro-detection.ts` and served via `/api/metro-areas/city-mapping` for the frontend. Both server and client use the same DB-driven mapping for vendor location detection. `server/utils/metro-resolver.ts` provides `resolveCustomCity()` and `resolveAreasServed()` helpers that check `metro_cities` first (case-insensitive), then `metro_areas`, and only create new entries as a last resort. All server routes (auth, weddings, vendors) use these helpers when handling custom cities, ensuring the DB is the single source of truth and the in-memory cache is refreshed after new entries.
